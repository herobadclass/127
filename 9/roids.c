...
void roid_draw( const roid_t* roid )
{
     ...
     */
    draw_rectangle( 
	    roid->x-roid->width/2.0 + 1, roid->y-roid->height/2.0, 
            roid->x+roid->width/2.0 + 1, roid->y+roid->height/2.0,
            roid->color );
    draw_rectangle( 
	    roid->x-roid->width/2.0, roid->y-roid->height/2.0 + 1, 
            roid->x+roid->width/2.0, roid->y+roid->height/2.0 + 1,
            roid->color );
    draw_rectangle( 
	    roid->x-roid->width/2.0 - 1, roid->y-roid->height/2.0, 
...
int shot_roid_hit( const shot_t* shot, const roid_t* roid )
{
	...
	if((x >= x1 && x <= x2) || ((x >= x1+1 || x <= x2) && x1 < 0) || (x2 > 1 && (x <= x2-1 || x >= x1)))
	{
		if((y >= y1 && y <= y2) || ((y <= y1-1 || y >= y2) && y1 > 1) || (y2 < 0 && (y >= y2+1 || y <= y1)))
		{
			return 1;
		}
	}
